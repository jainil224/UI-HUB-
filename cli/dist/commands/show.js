...
export const show = {
    ...
    async run(ctx, args) {
        ...
        if (detail?.description)
            renderKV('Description', detail.description);
        if (Array.isArray(deps?.dependencies) && deps.dependencies.length)
            renderKV('Dependencies', deps.dependencies.join(', '));
        if (detail?.installation)
            renderKV('Install', detail.installation);
        if (detail?.usageExample)
            renderKV('Usage', detail.usageExample);
        if (Array.isArray(detail?.tags) && detail.tags.length)
            renderKV('Tags', detail.tags.join(', '));
