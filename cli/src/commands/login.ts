...
function promptHidden(prompt: string): Promise<string> {
  if (!process.stdin.isTTY) {
    throw new CliError(
      'AUTH_ERROR',
      'Interactive login requires a terminal. Pass the key explicitly: ui-hub login --key uh_live_… (or set UI_HUB_API_KEY).',
      EXIT.AUTH,
    );
  }
...
export const login: Command = {
  ...
  async run(ctx: Ctx, args: ParsedArgs) {
    ...
    if (!key) key = await promptHidden('UI HUB API key (create one at ui-hub.onrender.com/dashboard/mcp): ');
    ...
    try {
      ...
      throw new CliError(
        err?.code ?? 'AUTH_ERROR',
        `${err?.message ?? 'Invalid API key.'} The key was not saved.`,
        EXIT.AUTH,
      );
    }
