...
async function main(): Promise<void> {
  ...
  if (needsKey && !cfg.apiKey) {
    throw new CliError(
      'AUTH_ERROR',
      'No UI HUB API key found. Authenticate first: "ui-hub login" — or set UI_HUB_API_KEY.',
      EXIT.AUTH,
    );
  }

  const client = new McpClient(cfg.endpoint, needsKey ? cfg.apiKey : undefined);
  const ctx: Ctx = { client, json, endpoint: cfg.endpoint, apiKey: cfg.apiKey };
...
main().catch((err) => {
  ...
  if (json) {
    ...
    if (code === 'USAGE_ERROR') console.error('Run "ui-hub help" for usage.');
  }
