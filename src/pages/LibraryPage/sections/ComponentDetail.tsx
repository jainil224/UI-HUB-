...
<${item.name.replace(/\s+/g, '')} />`;
    ...
    const componentConfig = COMPONENT_CONFIG[item.id] || {
        props: [],
        vibeMeta: { behavior: "", states: { from: "", to: "" }, cssProperties: [] }
    };

    const vibePrompt = generateVibePrompt(aiSystem, {
                        ...
                        <div className="aspect-video w-full glass rounded-[3rem] relative overflow-hidden flex items-center justify-center group bg-black/20 border border-white/5">
                            <div className="text-center w-full px-8">
                                <div className="flex justify-center">
                                    {(() => {
                                        const rawName = item.id.split('-').map((s: string) => s.charAt(0).toUpperCase() + s.slice(1)).join('');
                                        const CompName = item.id.endsWith('-text') ? rawName : `${rawName}Text`;

                                        // Try to find in Animations first, then VisualEffects, then by rawName
                    ...
                    >
                        ...
                        <section className="space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {(['antigravity', 'lovable', 'cursor'] as const).map(tool => (
                                    <button
                                        key={tool}
                                        onClick={() => setAiSystem(tool)}
                                        className={`p-8 rounded-[2rem] border transition-all text-left relative overflow-hidden group h-[180px] flex flex-col justify-between ${aiSystem === tool ? 'bg-brand-green/10 border-brand-green/30 shadow-[0_0_30px_rgba(0,255,0,0.1)]' : 'bg-white/5 border-white/10 hover:border-white/20'}`}
                                    >
                                        <div className="relative z-10">
                                            <div className="flex items-center justify-between mb-4">
                                                <p className={`text-[10px] uppercase tracking-widest font-bold ${aiSystem === tool ? 'text-brand-green' : 'text-white/40'}`}>
                                                    {tool === 'antigravity' ? 'Google' : tool === 'lovable' ? 'Platform' : 'IDE'}
                                                </p>
                                                <div className={`transition-all duration-300 ${aiSystem === tool ? 'text-brand-green scale-110' : 'text-white/20'}`}>
                                                    {tool === 'antigravity' ? (
                                            ...
                                            <h4 className="text-2xl font-display uppercase tracking-tight text-white">
                                        ...
                                        <div className={`mt-auto flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest transition-all duration-500 ${aiSystem === tool ? 'text-brand-green opacity-100' : 'text-white/20 opacity-0 group-hover:opacity-100'}`}>
                                            <div className={`w-1 h-1 rounded-full ${aiSystem === tool ? 'bg-brand-green animate-pulse' : 'bg-white/40'}`} />
                                            Active System
                                        </div>
                                    </button>
                                ))}
                        ...
                        <section className="space-y-8">
                            <div className="flex items-center justify-between px-2">
                                ...
                                <button
                                    onClick={() => handleCopy(vibePrompt, 'vibe')}
                            ...
                            <div className="glass rounded-[2.5rem] overflow-hidden border border-white/5 relative bg-black/40">
                                <div className="p-8 text-xs leading-relaxed max-h-[600px] overflow-auto">
                                    <pre className="font-sans whitespace-pre-wrap"><CodeHighlighter code={vibePrompt} /></pre>
                                </div>
                            </div>
                        </section>
                    </motion.div>
