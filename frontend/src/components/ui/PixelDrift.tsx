// PixelDrift.tsx — Interactive particle-text canvas (mouse-reactive, scroll-triggered)
...
function resolveEasingFn(trans?: TransitionConfig | null): EasingFn {
...
function resolveDuration(trans?: TransitionConfig | null): number {
...
const COMPONENT_DEFAULTS: Required<Omit<PixelDriftProps, 'className' | 'style'>> = {
    text: 'UI HUB',
...
export const PixelDrift = (props: PixelDriftProps) => {
    ...
    const mcRadius = typeof mouseRadius === 'number' ? mouseRadius : 65;
    ...
    useEffect(() => {
        ...
        const palette = Array.isArray(colors) && colors.length > 0 ? colors : ['#FFFFFF', '#F9731A', '#3D5CFF'];
        ...
        const sampleText = () => {
            ...
            if (autoFit) effectiveSize = fitFontSize(offCtx, text || '', fontFamily, maxW, maxH, Math.max(8, fontSize));
            ...
            count = i; ox = newOx; oy = newOy; sx = newSx; sy = newSy;
            px = newPx; py = newPy; repX = new Float32Array(allocCount); repY = new Float32Array(allocCount);
            cIdx = newC; formValRef.current = 0; lastFrameRef.current = null;
        };
        ...
        reverseRef.current = false; hiddenRef.current = true; formValRef.current = 0;
        ...
        const ro = new ResizeObserver(() => { resize(); if (tryEnter) tryEnter(); });
        ...
        const onMove = (e: PointerEvent) => {
            ...
            if (prevMx > -9000) {
                const ddx = mx - prevMx; const ddy = my - prevMy;
                mouseSpeed = Math.sqrt(ddx * ddx + ddy * ddy);
            }
            prevMx = mx; prevMy = my; pointerRef.current.x = mx; pointerRef.current.y = my; pointerRef.current.active = true;
        };
        const onLeave = () => {
        ...
        if (mode === 'onHover') {
            ...
            sentinel.style.position = 'absolute'; sentinel.style.left = '0';
            sentinel.style.width = '1px'; sentinel.style.height = '1px'; sentinel.style.pointerEvents = 'none';
            ...
            let entered = false;
            const enter = () => { if (entered) return; entered = true; formIn(); if (!replay && io) io.disconnect(); };
            tryEnter = () => {
                if (entered || typeof window === 'undefined') return;
                const r = container.getBoundingClientRect();
                if (r.width === 0 && r.height === 0) return;
                const vh = window.innerHeight || 0; const vw = window.innerWidth || 0;
                const y = position === 'middle' ? r.top + r.height / 2 : position === 'below' ? r.bottom : r.top;
                if (r.right >= 0 && r.left <= vw && r.bottom >= 0 && y <= vh) enter();
            };
            io = new IntersectionObserver(([entry]) => {
                if (entry.isIntersecting) enter();
                else if (replay) { entered = false; hiddenRef.current = true; reverseRef.current = false; formValRef.current = 0; }
            }, { threshold: 0 });
            if (sentinel) io.observe(sentinel);
            tryEnter();
            enterTimers.push(setTimeout(() => tryEnter && tryEnter(), 60), setTimeout(() => tryEnter && tryEnter(), 250), setTimeout(() => tryEnter && tryEnter(), 600));
        }
        ...
        const drawFrame = () => {
            ...
            for (let i = 0; i < count; i++) {
                ...
                let inZone = false;
                if (active) {
                    const dx = oxi - mx; const dy = oyi - my;
    ...
    return (
        <div ref={containerRef} className={className} style={{ position: 'relative', width: '100%', height: '100%', minHeight: '260px', overflow: 'hidden', userSelect: 'none', ...style }}>
            <canvas ref={canvasRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block' }} />
        </div>
    );
};

export default PixelDrift;
