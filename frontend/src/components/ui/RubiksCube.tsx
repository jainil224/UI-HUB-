...
const FC = {
    front: { bg: '#009B48', cls: styles.fcGreen },
    back: { bg: '#0051A2', cls: styles.fcBlue },
    right: { bg: '#C41E3A', cls: styles.fcRed },
    left: { bg: '#FF5800', cls: styles.fcOrange },
    top: { bg: '#FFFFFF', cls: styles.fcWhite },
    bottom: { bg: '#FFD500', cls: styles.fcYellow },
...
export const RubiksCube: React.FC = () => {
    ...
    const rotationRef = useRef({ x: -22, y: 45, velX: 0, velY: 0, lastDx: 0, lastDy: 0, dragging: false, lx2: 0, ly2: 0 });
    const manualModeRef = useRef(false);
    const manualTimerRef = useRef<any>(null);

    const makeCubie = useCallback((lx: number, ly: number, lz: number) => {
        const el = document.createElement('div');
        el.className = styles.cubie;
        FACE_DEFS.forEach(fd => {
            ...
            face.className = `${styles.cubieFace} ${fc.cls}`;
        ...
        // Round translations to the nearest STEP_PX
        m.m41 = Math.round(m.m41 / STEP_PX) * STEP_PX;
        m.m42 = Math.round(m.m42 / STEP_PX) * STEP_PX;
        m.m43 = Math.round(m.m43 / STEP_PX) * STEP_PX;

        // Round rotation components to -1, 0, or 1
        const keys = ['m11', 'm12', 'm13', 'm21', 'm22', 'm23', 'm31', 'm32', 'm33'];
        keys.forEach(k => {
            const v = (m as any)[k];
            if (Math.abs(v) < 0.1) (m as any)[k] = 0;
            else if (v > 0.9) (m as any)[k] = 1;
            else if (v < -0.9) (m as any)[k] = -1;
            else (m as any)[k] = Math.sign(v); // fallback
        });
    };
    ...
    const rotateLayer = useCallback(async (axis: string, slice: number, angle: number, ms: number) => {
        ...
        pivot.style.cssText = 'position:absolute;width:0;height:0;transform-style:preserve-3d;transition:none;';
        sceneRef.current.appendChild(pivot);
        layer.forEach(c => pivot.appendChild(c.el));

        pivot.getBoundingClientRect(); // flush
        ...
        const rotStr = axis === 'y' ? `rotateY(${angle}deg)` :
            axis === 'x' ? `rotateX(${angle}deg)` :
                `rotateZ(${angle}deg)`;
        pivot.style.transform = rotStr;

        await new Promise(resolve => setTimeout(resolve, ms + 80)); // Increased buffer
        ...
        layer.forEach(c => {
            ...
            snap(c.m);
            sceneRef.current?.appendChild(c.el);
    ...
    const scramble = useCallback(async (n = 14, ms = 220) => {
        if (busy) return;
        setBusy(true); setStatus('Scrambling...');
        historyRef.current = [];

        for (let i = 0; i < n; i++) {
            let m;
            do { m = MOVES[Math.floor(Math.random() * MOVES.length)]; }
            while (historyRef.current.length && historyRef.current[historyRef.current.length - 1].axis === m.axis && historyRef.current[historyRef.current.length - 1].slice === m.slice);
            historyRef.current.push(m);
            await rotateLayer(m.axis, m.slice, m.angle, ms);
            await new Promise(r => setTimeout(r, 40));
        }
        setBusy(false); setStatus('Scrambled — ready to solve');
    }, [busy, rotateLayer]);
    ...
    const solve = useCallback(async (ms = 380) => {
        ...
        historyRef.current = [];
        setBusy(false); setStatus('Solved! ✓');
    }, [busy, rotateLayer]);
    ...
    useEffect(() => {
        ...
        const animRot = () => {
            ...
            if (!rot.dragging) {
                rot.velY *= 0.92; rot.velX *= 0.92;
                if (!manualModeRef.current && historyRef.current.length === 0) {
                    rot.velY += (0.25 - rot.velY) * 0.025;
                    rot.velX += (0 - rot.velX) * 0.025;
                }
                rot.y += rot.velY;
                rot.x += rot.velX;
                rot.x = Math.max(-65, Math.min(65, rot.x));
            }
            if (sceneRef.current) {
                sceneRef.current.style.transform = `rotateX(${rot.x}deg) rotateY(${rot.y}deg)`;
            }
    ...
    const handleMouseDown = (e: React.MouseEvent | React.TouchEvent) => {
        const clientX = 'touches' in e ? e.touches[0].clientX : (e.nativeEvent instanceof MouseEvent ? (e.nativeEvent as MouseEvent).clientX : (e as any).clientX);
        const clientY = 'touches' in e ? e.touches[0].clientY : (e.nativeEvent instanceof MouseEvent ? (e.nativeEvent as MouseEvent).clientY : (e as any).clientY);
        const rot = rotationRef.current;
        rot.dragging = true; rot.lx2 = clientX || 0; rot.ly2 = clientY || 0;
        rot.velX = 0; rot.velY = 0; rot.lastDx = 0; rot.lastDy = 0;
    ...
    const handleMouseMove = useCallback((e: MouseEvent | TouchEvent) => {
        ...
        rot.lastDy = (clientY - rot.ly2) * 0.45;
        rot.y += rot.lastDx; rot.x -= rot.lastDy;
        rot.x = Math.max(-65, Math.min(65, rot.x));
    ...
    const handleMouseUp = useCallback(() => {
        ...
        if (!rot.dragging) return;
        rot.dragging = false;
        rot.velY = rot.lastDx * 0.85;
        rot.velX = -rot.lastDy * 0.85;
