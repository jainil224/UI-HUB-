...
interface HeroGlobelSplineProps {
    className?: string;
    onLoad?: () => void;
}

/**
 * HeroGlobelSpline Component
 * A premium 3D "HERO GLOBEL" animation experience powered by Spline.
 * Featuring a custom loader and smooth entry animations.
 */
export const HeroGlobelSpline: React.FC<HeroGlobelSplineProps> = ({
    className = "",
    onLoad
}) => {
        ...
        <div className={`relative w-full h-full min-h-[500px] overflow-hidden bg-[#050508] ${className} group/spline`}>
            ...
            <div className={`w-full h-full transition-opacity duration-1000 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
                <Suspense fallback={null}>
                    <Spline
                        scene="https://prod.spline.design/Eyw8Fsbzb1v07HLi/scene.splinecode"
                        onLoad={handleLoad}
                    />
                </Suspense>
            </div>
...
export default HeroGlobelSpline;
