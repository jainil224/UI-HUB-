...
interface OrbitalExperienceSplineProps {
    className?: string;
    onLoad?: () => void;
}

/**
 * OrbitalExperienceSpline Component
 * A premium 3D "Orbital Experience" animation experience powered by Spline.
 * Featuring a custom loader and smooth entry animations.
 */
export const OrbitalExperienceSpline: React.FC<OrbitalExperienceSplineProps> = ({
    className = "",
    onLoad
}) => {
    ...
    const handleLoad = (splineApp: any) => {
        ...
        try {
            objectsToHide.forEach(name => {
                ...
                if (obj && !obj.name.toLowerCase().includes('hub') && !obj.name.toLowerCase().includes('ui')) {
                    obj.visible = false;
                }
            });
        } catch (e) {
        ...
        <div className={`relative w-full h-full min-h-[500px] overflow-hidden bg-[#050508] ${className} group/spline`}>
                ...
                {isLoading && (
                    ...
                    >
                        ...
                        <div className="mt-12 flex flex-col items-center gap-3">
                            ...
                            >
                                Initializing Orbital Experience
                            </motion.p>
            ...
            <div className={`w-full h-full transition-opacity duration-1000 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
                <Suspense fallback={null}>
                    <Spline
                        scene="https://prod.spline.design/JeyZwSXuBPWf3ki3/scene.splinecode"
                        onLoad={handleLoad}
                    />
                </Suspense>
            </div>

            {/* Covering Div to hide Spline branding logo in bottom-right (Fallback if CSS fails) */}
            ...
            {!isLoading && (
                <motion.div
                    ...
                    className="absolute top-8 left-8 z-30 pointer-events-none flex items-center gap-4"
                >
...
export default OrbitalExperienceSpline;
