...
export const COMPONENT_CONFIG: Record<string, ComponentConfig> = {
    ...
    "card-cascade": {
        props: [
            { name: "preview", type: "boolean", default: "false", description: "Auto-advance through the cards when rendered inside a fixed-height preview box (no page scroll drives it)." },
            { name: "scrollerRef", type: "RefObject<HTMLElement | null>", default: "undefined", description: "Optional internal scroll container that drives the cascade in fullscreen mode; falls back to window scroll." }
        ],
        vibeMeta: {
            behavior: "A scroll-driven 3D arc cascade of six technical-skill cards arranged on a giant right-side semi-circle. As the page or internal container scrolls, cards travel along the arc toward a focal centre and tilt to the arc tangent, with eased card-to-card interpolation so motion glides instead of tracking scroll linearly. The active card rotates forward on its X axis to face the viewer while a vertical icon rail lists that card's skills. Preview mode auto-advances deterministically (ease, then hold, looping) because there is no page scroll. Mobile shrinks the arc radius, card size and rail.",
            states: { from: "cards stacked along the far arc with the header hidden", to: "active card facing forward at the arc apex with its skill rail expanded" },
            cssProperties: ["3d-transform", "perspective", "transform-style-preserve-3d", "sticky-positioning", "svg-pattern-grid", "clamp() sizing"],
            description: "A scroll-driven 3D semi-circular cascade of technical-skill cards with tangent tilt and per-card icon rails.",
            libraries: ["react", "react-icons", "lucide-react"],
            requirements: ["Semi-circle arc transform math (R·cos/R·sin + tangent rotation)", "rAF-throttled passive scroll handler with cleanup", "Deterministic preview auto-advance via cancelAnimationFrame", "useIsMobile-driven arc radius and clamp() card sizing", "3D perspective stage with preserve-3d cards"]
        }
    },

    "image-trail": {
        props: [
            { name: "images", type: "Array<string | ImageTrailImage>", default: "DEFAULT_IMAGES", description: "Images stamped along the pointer trail (strings or { src, alt })." },
            { name: "threshold", type: "number", default: "80", description: "Minimum pointer travel in px before a new trail image spawns." },
            { name: "minDelay", type: "number", default: "50", description: "Minimum ms between spawned trail images." },
            { name: "duration", type: "number", default: "1000", description: "Lifetime in ms before each trail image is removed." },
            { name: "maxItems", type: "number", default: "8", description: "Maximum simultaneous trail images." },
            { name: "rotationRange", type: "number", default: "40", description: "Total random rotation range in degrees (applied as ± half)." },
            { name: "imageClassName", type: "string", default: "undefined", description: "Extra classes for each trail image." },
            { name: "overlayClassName", type: "string", default: "undefined", description: "Extra classes for the pointer-events-none overlay layer." },
            { name: "transition", type: "Transition", default: "DEFAULT_TRANSITION", description: "Framer Motion transition for entering images (spring, stiffness 260, damping 22)." },
            { name: "exitTransition", type: "Transition", default: "DEFAULT_EXIT_TRANSITION", description: "Framer Motion transition for exiting images (0.4s easeInOut)." },
            { name: "disabled", type: "boolean", default: "false", description: "Disable spawning while still forwarding pointer handlers." }
        ],
        vibeMeta: {
            behavior: "A pointer-trail image effect: as the cursor moves across the container it stamps rotated photos that pop in with a spring and fade/scale out after a set duration. A distance threshold and a minimum delay throttle spawns, the image list cycles and maxItems caps the trail. Timeouts are tracked in a Set and cleared on unmount; images render inside a pointer-events-none overlay so underlying content stays interactive.",
            states: { from: "empty overlay", to: "trail of rotated image cards following the pointer" },
            cssProperties: ["absolute-positioning", "pointer-events-none-overlay", "aspect-ratio", "object-fit", "framer-motion-spring", "AnimatePresence"],
            description: "A cursor-following trail of rotated image cards spawned on pointer move with spring pop-in and timed fade-out.",
            libraries: ["react", "framer-motion"],
            requirements: ["Pointer-move throttling via distance threshold + min delay", "Cycling image index and maxItems cap", "Framer Motion AnimatePresence enter/exit", "Timeout Set cleared on unmount", "Inherits HTMLAttributes (className, children, pointer handlers)"]
        }
    },

    "perspective-carousel": {
        props: [
            { name: "items", type: "PerspectiveCarouselItem[]", default: "DEFAULT_ITEMS", description: "Slides as { src, title, alt? }." },
            { name: "activeIndex", type: "number", default: "undefined", description: "Controlled active slide index (uncontrolled when omitted)." },
            { name: "defaultActiveIndex", type: "number", default: "2", description: "Initial active slide for uncontrolled use." },
            { name: "onActiveIndexChange", type: "(index: number) => void", default: "undefined", description: "Called whenever the active slide changes." },
            { name: "loop", type: "boolean", default: "true", description: "Wrap navigation around the ends." },
            { name: "slideWidth", type: "number", default: "220", description: "Slide width in px (clamped to >= 96)." },
            { name: "rotationStep", type: "number", default: "55", description: "rotateY degrees applied per slide of distance from the active card." },
            { name: "inactiveScale", type: "number", default: "0.85", description: "Scale of inactive slides (clamped 0.5-1)." },
            { name: "transition", type: "Transition", default: "DEFAULT_TRANSITION", description: "Framer Motion transition (spring, bounce 0.14, duration 0.9)." },
            { name: "showControls", type: "boolean", default: "true", description: "Show the prev/next control bar." },
            { name: "showDots", type: "boolean", default: "true", description: "Show dot pagination inside the control bar." },
            { name: "viewportClassName", type: "string", default: "undefined", description: "Extra classes for the perspective viewport." },
            { name: "slideClassName", type: "string", default: "undefined", description: "Extra classes for each slide." },
            { name: "imageClassName", type: "string", default: "undefined", description: "Extra classes for slide images." },
            { name: "labelClassName", type: "string", default: "undefined", description: "Extra classes for slide title labels." },
            { name: "controlsClassName", type: "string", default: "undefined", description: "Extra classes for the control bar." }
        ],
        vibeMeta: {
            behavior: "A 3D perspective carousel where slides fan around a shared vertical axis: the active card faces forward while neighbouring cards rotateY by rotationStep per unit distance, scale down and fade. The track translates horizontally to centre the active slide. Navigation via clickable cards, prev/next buttons, dot pagination and ArrowLeft/ArrowRight keys; loop wraps indices. Supports controlled or uncontrolled activeIndex and returns null when there are no items.",
            states: { from: "slides fanned in perspective", to: "selected slide rotated forward and centred with its label focused" },
            cssProperties: ["perspective", "rotateY", "preserve-3d", "will-change-transform", "backdrop-blur", "framer-motion-spring"],
            description: "A 3D perspective image carousel with rotateY slide fan, loop, dots and keyboard navigation.",
            libraries: ["react", "framer-motion", "lucide-react"],
            requirements: ["Per-slide rotateY fan + active scale/opacity", "Controlled/uncontrolled activeIndex with onActiveIndexChange", "Keyboard ArrowLeft/ArrowRight handling", "ARIA carousel region + aria-current slides", "Inherits HTMLAttributes (className etc.)"]
        }
    },

    "diagonal-carousel": {
        props: [
            { name: "items", type: "DiagonalCarouselItem[]", default: "DEFAULT_ITEMS", description: "Slides as { src, title, alt? }." },
            { name: "activeIndex", type: "number", default: "undefined", description: "Controlled active slide index (uncontrolled when omitted)." },
            { name: "defaultActiveIndex", type: "number", default: "2", description: "Initial active slide for uncontrolled use." },
            { name: "onActiveIndexChange", type: "(index: number) => void", default: "undefined", description: "Called whenever the active slide changes." },
            { name: "loop", type: "boolean", default: "true", description: "Wrap navigation around the ends." },
            { name: "slideSize", type: "number", default: "260", description: "Slide size in px (clamped to >= 120)." },
            { name: "rotationStep", type: "number", default: "24", description: "Rotation degrees applied per slide of distance from the active card." },
            { name: "verticalStep", type: "number", default: "100", description: "Vertical offset in px per slide of distance from the active card." },
            { name: "inactiveScale", type: "number", default: "0.65", description: "Scale of inactive slides (clamped 0.35-1)." },
            { name: "transition", type: "Transition", default: "DEFAULT_TRANSITION", description: "Framer Motion transition (spring, bounce 0.16, duration 0.85)." },
            { name: "showControls", type: "boolean", default: "true", description: "Show the prev/next control bar." },
            { name: "showDots", type: "boolean", default: "true", description: "Show dot pagination inside the control bar." },
            { name: "viewportClassName", type: "string", default: "undefined", description: "Extra classes for the viewport." },
            { name: "slideClassName", type: "string", default: "undefined", description: "Extra classes for each slide." },
            { name: "imageClassName", type: "string", default: "undefined", description: "Extra classes for slide images." },
            { name: "labelClassName", type: "string", default: "undefined", description: "Extra classes for slide title labels." },
            { name: "controlsClassName", type: "string", default: "undefined", description: "Extra classes for the control bar." }
        ...
        vibeMeta: {
            behavior: "A diagonal 3D carousel: slides march down an angled line with each neighbour offset vertically by verticalStep, rotated by rotationStep and scaled down, while the active square card sits sharp and bright. The track translates horizontally to centre the active slide. Navigation via card click, prev/next buttons, dots and ArrowLeft/ArrowRight; loop wraps. Supports controlled or uncontrolled activeIndex and returns null when there are no items.",
            states: { from: "slides stepped down the diagonal", to: "active card centred at the apex with its label scaled in" },
            cssProperties: ["translate", "rotate", "scale", "will-change-transform", "backdrop-blur", "framer-motion-spring"],
            description: "A diagonal stepped image carousel with vertical offset, rotation, loop and keyboard navigation.",
            libraries: ["react", "framer-motion", "lucide-react"],
            requirements: ["Diagonal step math (x track + y = distance*verticalStep + rotation)", "Controlled/uncontrolled activeIndex", "Keyboard ArrowLeft/ArrowRight handling", "ARIA carousel region + aria-current slides", "Inherits HTMLAttributes"]
        }
    },

    "testimonials-card": {
        props: [
            { name: "items", type: "TestimonialItem[]", default: "DEFAULT_TESTIMONIALS", description: "Testimonials as { id, title, description, image }." },
            { name: "className", type: "string", default: "undefined", description: "Extra classes for the outer container." },
            { name: "width", type: "number", default: "560", description: "Max width of the card stack in px." },
            { name: "showNavigation", type: "boolean", default: "true", description: "Show the previous/next arrow controls." },
            { name: "showCounter", type: "boolean", default: "true", description: "Show the active/total counter label." },
            { name: "autoPlay", type: "boolean", default: "false", description: "Advance automatically on an interval." },
            { name: "autoPlayInterval", type: "number", default: "3500", description: "Auto-play interval in ms." }
        ],
        vibeMeta: {
            behavior: "A 3D testimonial card stack: the active portrait pops forward out of a fanned stack of image cards (rotateZ offsets, z depth, scale) while its name and quote animate in; direction-aware enter/exit slides the card in from the off-screen side. A counter shows position, arrow buttons navigate with disabled ends, and optional autoplay advances on an interval cleared on unmount. Collapses to a single column under md.",
            states: { from: "stacked portrait cards with the first active", to: "active portrait pulled forward and its testimonial text cross-faded in" },
            cssProperties: ["3d-transform", "perspective", "z-index-stacking", "AnimatePresence", "ease-[0.22,1,0.36,1]"],
            description: "A 3D stacked portrait testimonial slider with cross-faded quote, counter and optional autoplay.",
            libraries: ["react", "framer-motion", "lucide-react"],
            requirements: ["Direction-aware AnimatePresence enter/exit", "Fanned stack transforms (rotateZ/z/scale per offset)", "Autoplay interval cleared on unmount", "ARIA labels on nav buttons", "Responsive single-column layout under md"]
        }
    },

    "image-collage": {
        props: [
            { name: "images", type: "CollageImage[]", default: "DEFAULT_COLLAGE_IMAGES", description: "Images as { src, x, y, rotate, alt? }." },
            { name: "containerClassName", type: "string", default: "undefined", description: "Extra classes for the inner card row." },
            { name: "imageClassName", type: "string", default: "undefined", description: "Extra classes for each image card." }
        ],
        vibeMeta: {
            behavior: "An interactive image collage that toggles between a scattered cluster and an organised row on click/tap. Each card springs between its authored x/y/rotate offsets and a flat aligned row using a stiff spring (stiffness 350, damping 25, bounce 0.4); the caption switches between 'Dynamic Collage Cluster' and 'Organized Row View'. Forwards refs and spreads HTML attributes so it can be placed anywhere.",
            states: { from: "scattered rotated cluster of overlapping cards", to: "flat evenly overlapped aligned row" },
            cssProperties: ["negative-margin-overlap", "framer-motion-spring", "aspect-ratio", "object-fit", "cursor-pointer"],
            description: "A click-to-toggle image collage that springs between a scattered cluster and an aligned row.",
            libraries: ["react", "framer-motion"],
            requirements: ["Per-card spring animate between authored offsets and zeroed row", "React.forwardRef + HTMLAttributes passthrough", "Click toggle of the isOrganized state", "Responsive card widths + negative overlap", "No rAF or event listeners"]
        }
    },

    "image-compare": {
        props: [
            { name: "before", type: "ImageSource", default: "IMAGE_COMPARE_DEFAULTS.before", description: "\"Before\" image { src, alt? } shown under the reveal layer." },
            { name: "after", type: "ImageSource", default: "IMAGE_COMPARE_DEFAULTS.after", description: "\"After\" image { src, alt? } shown as the base layer." },
            { name: "initialPosition", type: "number", default: "50", description: "Starting divider position 0-100 (%)." },
            { name: "orientation", type: "'horizontal' | 'vertical'", default: "'horizontal'", description: "Drag axis; horizontal splits left/right, vertical splits top/bottom." },
            { name: "showLabels", type: "boolean", default: "true", description: "Show the Before/After chips." },
            { name: "labelBefore", type: "string", default: "'Before'", description: "Chip text + alt fallback for the before image." },
            { name: "labelAfter", type: "string", default: "'After'", description: "Chip text + alt fallback for the after image." },
            { name: "ariaLabel", type: "string", default: "'Before and after image comparison'", description: "Accessible name of the slider widget." },
            ...
            { name: "onPositionChange", type: "(pos: number) => void", default: "undefined", description: "Called with the rounded 0-100 position while the divider moves." }
        ],
        vibeMeta: {
            behavior: "A before/after slider: a full-size after image sits on top of a clip-path-inset masked before copy; the divider edge and grip follow the pointer (or arrow keys) through a stiffness-900/damping-55 spring, so the reveal decelerates into place instead of snapping.",
            states: { from: "divider at start position over a still image", to: "divider slides to reveal the other state" },
            cssProperties: ["clip-path: inset()", "object-fit: cover", "framer-motion spring", "aspect-ratio", "touch-action: none"],
            description: "A draggable divider that peels between before/after image states with spring physics and ARIA slider semantics.",
            libraries: ["react", "framer-motion"],
            requirements: ["clip-path inset() reveal, no image re-layout", "stiff 900/55 spring on divider + edge", "pointer + arrow-key control, Home/End jump", "role=slider with live aria-valuenow", "prefers-reduced-motion jumps instead of springs", "no state writes per pointer frame"]
        }
    },

    "image-lens": {
        props: [
            { name: "image", type: "ImageSource", default: "IMAGE_LENS_DEFAULTS.image", description: "Source image { src, alt? } to magnify." },
            { name: "zoom", type: "number", default: "3", description: "Magnification power, clamped 1.5-6." },
            { name: "lensSize", type: "number", default: "160", description: "Lens diameter in px (min 48)." },
            { name: "cornerRadius", type: "number", default: "999", description: "Lens corner radius in px; near-full circle by default, 0 for a square lens." },
            { name: "borderColor", type: "string", default: "rgba(255,255,255,0.35)", description: "Color of the crisp box-shadow lens ring." },
            { name: "followSmoothing", type: "number", default: "80", description: "Lens follow lag in ms; 0 = instant." },
            { name: "alwaysOn", type: "boolean", default: "false", description: "Keep the lens visible even before hover." },
            { name: "disabled", type: "boolean", default: "false", description: "Disable the lens entirely and hide it." },
            { name: "className", type: "string", default: "''", description: "Extra classes for the wrapper." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Wrapper style escape hatch." }
        ],
        vibeMeta: {
            behavior: "A hover lens: a circular, pointer-events-none overlay tracks the clamped cursor; an inner copy of the base image is scaled ~3x (1.5-6) and translated so the pixel under the lens centre lands on the lens centre; a shared CSS transform transition gives the follower a short lag with zero JS animation loop; leaving fades the lens out. The lens ring is rendered as a box-shadow so it stays crisp on the rounded clip while following, and the magnified layer opts into will-change transform to avoid shimmer. The default preview image is a supplied lava artwork URL shared by the base image and the magnified layer.",
            states: { from: "pristine image, no lens", to: "circular lens tracks the pointer, magnifying the region beneath it" },
            cssProperties: ["transform: translate + scale", "background-image cover layer", "box-shadow lens ring + border-radius clip", "overflow-hidden", "touch-action: none"],
            description: "A hover-following circular lens that magnifies an image region up to 6x with zero per-frame state.",
            libraries: ["react"],
            requirements: ["POINTER-follow with clamped lens position", "magnified inner layer pinned under lens centre", "ResizeObserver sizing, disconnected on unmount", "no state writes per pointer frame", "prefers-reduced-motion disables the easing transition", "lens ring is a box-shadow (crisp on rounded clip, no dotted edge)", "lens + inner layers use will-change transform to prevent shimmer", "default preview image URL is shared by the base <img> and the lens background layer"]
        }
    },
    ...
    "scroll-expand": {
        props: [
            ...
            { name: "children", type: "React.ReactNode", default: "undefined", description: "Content that fades in over the media once it reaches full bleed." },
            { name: "className", type: "string", default: '""', description: "Additional class names for the container." },
            { name: "style", type: "object", default: "undefined", description: "Inline styles for the container." }
        ],
        vibeMeta: {
            behavior: "A scroll-triggered media frame expansion effect. Starts as a centered, rounded frame with a title overlay. As you scroll, the frame expands to full viewport, corners sharpen, title fades out, and overlay content fades in.",
            states: { from: "centered rounded frame with title", to: "full bleed media with overlay content" },
            cssProperties: ["clip-path", "transform: scale", "opacity", "requestAnimationFrame"],
            description: "Scroll-driven image/video expansion with smooth easing, configurable dimensions, and overlay content support.",
            libraries: ["react"],
    ...
    "reflect-shader": {
        props: [
            { name: "background", type: "string", default: '"#020202"', description: "CSS background colour behind the canvas." },
            { name: "tint", type: "string", default: '"#ffffff"', description: "RGB multiplier applied to the final colour." },
            { name: "speed", type: "number", default: "50", description: "Animation rate (0 = frozen, 100 = fast)." },
            { name: "brightness", type: "number", default: "100", description: "Shader uniform intensity multiplier." },
            { name: "thickness", type: "number", default: "20", description: "Refractive band thickness (uLineWidth)." },
            { name: "chromatic", type: "number", default: "10", description: "Channel rotation rate (uSpread) controlling chromatic offset." },
            { name: "bandGap", type: "number", default: "20", description: "Spatial frequency of the banding (uBands)." },
            { name: "zoom", type: "number", default: "295", description: "View scale (uScale); higher = zoomed in." },
            { name: "hover", type: "number", default: "90", description: "Pointer glow intensity / reach (uHover)." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Optional outer wrapper style override." }
        ],
        vibeMeta: {
            behavior: "A chromium-refraction WebGL shader field composed of 12 thin lines whose per-channel rotations produce shifting chromatic bands that bloom brighter where the pointer rests. Pointer tracking is eased with lerp + dt so the glow follows smoothly without jumps; resize and DPR are handled with ResizeObserver + Math.min(devicePixelRatio, 2).",
            states: { from: "static chromatic field at rest", to: "pointer-following glow bloom over animated refraction bands" },
            cssProperties: ["webgl", "canvas", "fullscreen background", "additive-like glow via alpha", "lerp pointer easing"],
            description: "Chromium-refraction WebGL shader field with pointer-following glow and chromatic band rotation.",
            libraries: ["react"],
            requirements: ["WebGL 1 context", "requestAnimationFrame loop", "ResizeObserver", "devicePixelRatio scaling", "lerp pointer easing"]
        }
    },
    "infinite-tendrils": {
        props: [
            { name: "colorA", type: "string", default: '""', description: "First colour used in the per-mote gradient (defaults to gray)." },
            { name: "colorB", type: "string", default: '""', description: "Second colour used in the per-mote gradient (defaults to gray)." },
            { name: "count", type: "number", default: "18", description: "Tendril density 1-20 (maps to 90-1390 motes)." },
            { name: "scale", type: "number", default: "14", description: "Noise-field spatial scale 1-20." },
            { name: "size", type: "number", default: "5", description: "Line width 1-20." },
            { name: "trail", type: "number", default: "20", description: "Trail persistence 1-20 (higher = longer)." },
            { name: "speed", type: "number", default: "100", description: "Travel rate 0-20 (clamped, 0 = still)." },
            { name: "followPointer", type: "boolean", default: "true", description: "Whether tendrils curl toward the pointer." },
            { name: "strength", type: "number", default: "13", description: "Pointer grip reach and whirl 1-20." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Optional outer wrapper style override." }
        ],
        vibeMeta: {
            behavior: "A swarm of motes tracing infinite tendrils through a Canvas 2D field of angles. Each heading is steered by two scrolling value-noise octaves (broad swell + fine fray) while motes move at per-mote pace and leave a fading trail via destination-out compositing. Per-mote colors mix colorA/colorB as an rgba gradient faded at trail ends; with followPointer the pointer grips nearby motes with quadratic-falloff pull so tendrils curl around the cursor.",
            states: { from: "tendrils drifting on noise currents", to: "tendrils curling around the pointer with eased grip" },
            cssProperties: ["canvas 2d", "destination-out trail fade", "value-noise flow field", "rgba gradient strokes", "dpr-capped resize"],
            description: "Canvas flow-field of glowing particle tendrils that curl toward your pointer.",
            libraries: ["react"],
            requirements: ["Canvas 2D", "requestAnimationFrame loop", "ResizeObserver", "value noise (2 octaves)", "destination-out trail", "pointer grip easing", "devicePixelRatio cap"]
        }
    },
    "ocean-swell": {
        props: [
            { name: "deep", type: "string", default: '"#04222E"', description: "Deep water colour." },
            { name: "shallow", type: "string", default: '"#1E7F86"', description: "Shallow/crest water colour." },
            { name: "scatter", type: "string", default: '"#CCCFCE"', description: "Subsurface scattering colour near crests." },
            { name: "foam", type: "string", default: '"#F2FAFF"', description: "Foam colour on steep slopes." },
            { name: "zenith", type: "string", default: '"#2C5C8E"', description: "Sky zenith colour." },
            { name: "horizon", type: "string", default: '"#FAF5B4"', description: "Sky horizon colour." },
            { name: "swell", type: "number", default: "1", description: "Wave amplitude 1-20." },
            { name: "choppy", type: "number", default: "20", description: "Gerstner horizontal displacement 0-20." },
            { name: "detail", type: "number", default: "20", description: "Wave frequency base 1-20." },
            { name: "glitter", type: "number", default: "20", description: "Specular sharpness 1-20." },
            { name: "subsurface", type: "number", default: "20", description: "Scatter strength 0-20." },
            { name: "foamAmount", type: "number", default: "20", description: "Foam coverage 0-20." },
            { name: "cloud", type: "number", default: "20", description: "Cloud deck density 0-20." },
            { name: "haze", type: "number", default: "0", description: "Atmospheric haze 0-20." },
            { name: "speed", type: "number", default: "5", description: "Simulation speed 0-20." },
            { name: "drag", type: "number", default: "20", description: "Drag-sensitivity of the orbit 0-20 (0 = no drag)." },
            { name: "sizePercent", type: "number", default: "200", description: "Focal/zoom 40-200." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Optional outer wrapper style override." }
        ],
        vibeMeta: {
            behavior: "A ray-marched procedural ocean on a fullscreen THREE shader quad. 4 gerstner-style waves drive a 68-step crest trace with adaptive stepping; 7 waves produce analytic normals for fresnel reflections of a value-noise-cloud sky, sharp glitter speculars, subsurface scattering near crests, and value-noise foam on steep slopes, closed with haze fog and a filmic tonemap. The camera auto-drifts and orbits on drag with inertia decay, clamped to a max pitch. DPR-capped at 1.25.",
            states: { from: "calm open ocean with sun glints", to: "drag-orbit camera sweeping over lit swell" },
            cssProperties: ["webgl", "shader quad", "ray-march", "fresnel", "filmic tonemap", "drag inertia"],
            description: "Ray-marched procedural ocean with sun glitter, foam crests, and drag-to-orbit camera.",
            libraries: ["react", "three"],
            requirements: ["THREE.WebGLRenderer", "ShaderMaterial", "68-step ray march", "4-wave crest + 7-wave detail traces", "gerstner displacement", "fresnel + sky reflection", "glitter specular", "subsurface + foam", "drag inertia camera", "devicePixelRatio cap"]
        }
    },
    "frost-glass-melt": {
        props: [
            { name: "background", type: "string", default: '"#060A14"', description: "Base background colour behind the glass." },
            { name: "frost", type: "string", default: '"#D9E6FF"', description: "Frost crystal colour." },
            { name: "scene", type: "number", default: "20", description: "Snow-mote density behind the glass 0-20 (maps to 0-150 motes)." },
            { name: "cell", type: "number", default: "12", description: "Frost granularity 1-20 (maps to 6-32px cells)." },
            { name: "meltRadius", type: "number", default: "14", description: "Pointer thaw reach in px 1-20." },
            { name: "meltSpeed", type: "number", default: "12", description: "How quickly frost melts under the pointer heat 1-20." },
            { name: "refreezeSpeed", type: "number", default: "8", description: "How quickly frost re-forms when idle 1-20." },
            { name: "drift", type: "number", default: "10", description: "Snow drift / wind speed 0-20 (0 = still)." },
            { name: "twinkle", type: "number", default: "12", description: "Crystal shimmer rate 0-20." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Optional outer wrapper style override." }
        ],
        vibeMeta: {
            behavior: "A frosted-glass pane on a fullscreen Canvas 2D: a per-cell frost grid sits over an animated winter scene (radial gradient + drifting snow motes with sine sway and edge wrap). The pointer radiates heat into a diffusing 3x3 heat buffer with inverse-square falloff; heat lowers frost levels to reveal the scene and Click banks a 1.4x burst. Idle, heat decays so frost slowly re-freezes and the pane self-heals. Each frost cell renders with a twinkling value-noise alpha grain. DPR-capped at 2; prefers-reduced-motion renders one static frosted frame; touch-action none.",
            states: { from: "fully frosted pane with shimmering crystals", to: "pointer-thawed clear window revealing the drifting snow scene" },
            cssProperties: ["canvas 2d", "fullscreen background", "per-cell frost grid", "diffusive heat field", "value-noise twinkle", "dpr-capped resize"],
            description: "A frosted glass pane that melts around your pointer to reveal a winter scene and re-freezes when idle.",
            libraries: ["react"],
            requirements: ["Canvas 2D", "requestAnimationFrame loop", "ResizeObserver", "per-cell frost + heat buffers", "3x3 heat diffusion", "snow-mote scene layer", "pointer heat + click burst", "value-noise twinkle", "devicePixelRatio cap", "prefers-reduced-motion support"]
        }
    },
    "star-burst": {
        props: [
            { name: "speed", type: "number", default: "10", description: "Outward travel rate 0-100." },
            { name: "starCount", type: "number", default: "100", description: "Number of spokes (each carries 15 pulses, capped at 5000 particles)." },
            { name: "color", type: "string", default: '"#FFFFFF"', description: "Star streak colour (hex or rgba)." },
            { name: "centerX", type: "number", default: "50", description: "Burst centre X 0-100 (% of width)." },
            { name: "centerY", type: "number", default: "100", description: "Burst centre Y 0-100 (% of height)." },
            { name: "starSize", type: "number", default: "12", description: "Streak length scale 0-20." },
            { name: "opacity", type: "number", default: "50", description: "Global fade 0-100." },
            { name: "flowerIntensity", type: "number", default: "10", description: "Radial bloom strength + per-particle alpha 0-20." },
            { name: "twinkleSpeed", type: "number", default: "4", description: "Sine twinkle rate 0-20." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Optional outer wrapper style override." }
        ],
        vibeMeta: {
            behavior: "A radial flower-burst on Canvas 2D: starCount jittered spokes each launch 15 pulses that fly outward, drawn additively as rotated streak sprites (lighter compositing) so the trails fan into a star flower. Particles twinkle on a per-phase sine, fade at spawn/trail end, and respawn behind the core; a radial gradient center bloom scales with flowerIntensity/starSize. Each frame clears to black so it sits over any page. DPR-capped at 2; export/thumbnail modes render 60 static frames.",
            states: { from: "concealed core with faint bloom", to: "full radial flower of twinkling streaks" },
            cssProperties: ["canvas 2d", "additive blending", "rotated streak sprites", "radial gradient bloom", "seeded rng", "dpr-capped resize"],
            description: "Additive canvas flower-burst of twinkling star streaks blooming from a glowing core.",
            libraries: ["react"],
    ...
    "sky": {
        props: [
            ...
            { name: "stars", type: "boolean", default: "true", description: "Whether twinkling stars render in the upper sky." },
            { name: "starCount", type: "number", default: "130", description: "Number of stars (0-400)." },
            { name: "twinkleSpeed", type: "number", default: "6", description: "Star twinkle rate 0-20." },
            { name: "clouds", type: "boolean", default: "true", description: "Whether drifting cumulus clouds render." },
            { name: "cloudCount", type: "number", default: "5", description: "Number of clouds (0-12)." },
            { name: "cloudSpeed", type: "number", default: "8", description: "Downwind cloud drift speed 0-20." },
            { name: "parallax", type: "boolean", default: "true", description: "Whether the pointer eases a parallax between layers." },
            { name: "parallaxStrength", type: "number", default: "16", description: "Parallax offset strength in px 0-80." },
            { name: "seed", type: "number", default: "1", description: "Seed for reproducible star/cloud placement." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Optional outer wrapper style override." },
            { name: "className", type: "string", default: '""', description: "Optional extra Tailwind classes for the wrapper." }
        ],
        vibeMeta: {
            behavior: "A Canvas 2D atmospheric sky: a vertical multi-stop gradient horizon built from three palettes (day: bright blue with faint stars; dusk: violet + orange with a low warm sun; night: deep navy with a bright moon), a warm sun or pale moon rendered from a cached 560x560 offscreen radial-gradient sprite (hot white core + outer falloff haze), a seeded field of 130 twinkling stars concentrated toward the zenith (pow(rng,1.3)*0.85 high-altitude distribution; twinkle = 0.35 + 0.65 * max(0, sin(time * twinkleSpeed * 0.4 * speed + phase)) cosine curve with halo for bright stars), and soft cumulus clouds pre-baked into offscreen 480x220 puff sprites (8-12 overlapping radial-gradient blobs tinted per palette) that drift at dt * speed * (cloudSpeed*2.5 + 6), self-wrap at width + w, and draw a mirrored edge copy when overflowing. Pointer parallax uses a normalized target clamped to [-1,1] with exponential lerp (1 - exp(-4*dt)) across layers: stars 0.2x, orb 0.45x, clouds (0.5 + depth * 0.7)x. DPR capped at 2 (1.5 on canvases >1920px) with setTransform reset on ResizeObserver, cloud x positions re-homed if they pass width*1.5, IntersectionObserver parks the rAF loop off-screen, and prefers-reduced-motion cancels the loop, zeroes parallax and draws one static frame. The decorative canvas is pointer-events none and carries an aria-label describing the active palette.",
            states: { from: "calm gradient horizon with drifting clouds", to: "pointer-eased parallax shifting the cloud, star and glow layers at different depths" },
            cssProperties: ["canvas 2d", "cached 560x560 orb glow sprite", "cached 480x220 cloud puff sprites", "multi-stop vertical gradient", "seeded rng", "exponential lerp pointer parallax", "cell wrap + mirrored edge cloud draw", "dpr-capped resize", "intersection observer parking"],
            description: "A serene animated sky with a glowing sun or moon, twinkling stars and drifting cumulus clouds eased by pointer parallax.",
            libraries: ["react"],
requirements: ["Canvas 2D", "requestAnimationFrame loop", "ResizeObserver", "IntersectionObserver parking", "seeded rng", "cached orb + cloud sprites", "exponential lerp pointer parallax", "devicePixelRatio cap", "prefers-reduced-motion support"]
        }
    },
    "vector-wordmark": {
        props: [
            { name: "text", type: "string", default: '"UI HUB"', description: "Brand text rendered on the wordmark canvas." },
            { name: "font", type: "React.CSSProperties", default: 'Inter 800 / 200px / -0.02em', description: "Font spec: fontFamily, fontWeight, fontSize, fontStyle, letterSpacing (fontSize scales with host width)." },
            { name: "background", type: "string", default: '"#000000"', description: "Host background color behind the canvas." },
            { name: "textColor", type: "string", default: '"#FFFFFF"', description: "Text fill color (hex / rgb / hsl / var())." },
            { name: "shade", type: "string", default: '"#FFFFFF"', description: "Bottom-to-top shade tint mixed into the fill." },
            { name: "accent", type: "string", default: '"#FFFFFF"', description: "Handle/label accent color for the dashed frame, boxes and labels." },
            { name: "reach", type: "number", default: "290", description: "Pointer reach radius in px; larger = handles dart from farther away." },
            { name: "speed", type: "number", default: "50", description: "Overall motion rate (idle sweep + drift), 0-100." },
            { name: "damping", type: "number", default: "60", description: "Pointer easing damp factor, 0-100." },
            { name: "handles", type: "HandleGroup", default: '{ size: 109, spread: 27, labels: true }', description: "Handle group config: size (frame half-size), spread (cell spacing), labels (show x/y % readouts)." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Wrapper style escape hatch." }
        ],
        vibeMeta: {
            behavior: "A raw-WebGL typographic wordmark (single GL context, one fullscreen-triangle draw, live state via refs). The text is rasterized once into an offscreen font atlas: a red fill plus a dashed green perimeter stroke laid into a black RGBA canvas, scaled to fit the 4096px texture cap with a 0.12em pad, then uploaded as a texture with mipmaps (POT or WebGL2) and re-baked whenever text/font/dpr/size changes. The fragment shader samples the atlas with a 6-direction blur to soften the stem, mixes sharp and soft reads by a pointer-distance falloff (k = 1 - smoothstep(0, reach, d)^3) so the wordmark sharpens under the cursor, blends a bottom-up shade gradient into the text color, and overlays an accent dashed triangle plus three dashed bounding boxes (uV0/uV1/uV2 + uHalf) around three drifting vertices. Pointer-aware motion: without a pointer the framerate viewport sweeps left-to-right at speed with constant y at the text baseline and a tiny z-noise sway; on pointermove the target snaps to the nearest cell centers around the cursor inside a 0.6-aspect grid, two vertices hitch cells while the third holds the pointer cell, and every vertex orbits its cell with per-cell pseudo-random sine drift (frequency 1.3 / 1.69) offset by a hash of the cell coords. Easing is an exponential damp (damping/100 * 20 * dt) toward the target. The host keeps a minWidth of 1200 and minHeight of 800, canvas DPR-capped at 2, rAF loop gated on document.visibilitychange, and the active hover reach/framing readouts are drawn as monospace labels at each handle.",
            states: { from: "a softly blurred wordmark on black with three dimension handles drifting on sine currents", to: "the text sharpens and the handles dart to nearest cell centers as the pointer moves, with live % readouts" },
            cssProperties: ["raw webgl", "webgl2 with webgl1 fallback", "offscreen font atlas texture", "6-direction blur sampling", "dashed accent frames + boxes", "radial pointer-distance falloff", "cell-grid snapping", "per-cell hash noise drift", "exponential damp easing", "dpr cap", "visibility gate", "resize observer"],
            description: "A raw-WebGL typographic wordmark stage for brand text: soft-blur stems that sharpen around the pointer with three dotted dimension handles that snap, drift, and trail the cursor inside a cell grid, plus a dashed targeting frame.",
            libraries: ["react"],
            requirements: ["WebGL (WebGL2 preferred, WebGL1 fallback)", "Canvas 2D font atlas", "requestAnimationFrame loop", "ResizeObserver + devicePixelRatio cap", "document.fonts readiness", "visibilitychange rAF gate"]
        }
    },
    "ember-husk": {
        props: [
            { name: "background", type: "EmberHuskBackground", default: '{ color: "#1A0402", glowColor: "#FF2606", glow: 6 }', description: "Scene backdrop: color (clear + fog tint), glowColor (spill light), glow (radiance 0-10)." },
            { name: "size", type: "number", default: "3.7", description: "Global scale of the husk, its shards, orbits and motion." },
            { name: "structure", type: "EmberHuskStructure", default: '{ shards: 11, irregularity: 6, thickness: 5 }', description: "Husk shell: shard count, irregularity (fracture variance 0-10), thickness (shell depth 0-10)." },
            { name: "rock", type: "EmberHuskRock", default: '{ color: "#171512", weathering: 5, seam: 4 }', description: "Shard material: base color, weathering (surface noise 0-10), seam (crack glow weight 0-10)." },
            { name: "core", type: "EmberHuskCore", default: '{ color: "#FF3B00", hotColor: "#FFC46A", intensity: 6, flow: 5 }', description: "Molten core: color, hotColor, intensity (0-10), flow (fluid wobble 0-10)." },
            { name: "crosses", type: "EmberHuskCrosses", default: '{ count: 52, size: 5, thickness: 5, color: "#EDEFF5", spread: 5 }', description: "Marker crosses: orbiting beacon count, size, thickness, color, orbital spread." },
            { name: "debris", type: "EmberHuskDebris", default: '{ count: 54, size: 5, spread: 5 }', description: "Free-floating debris field: particle count, size, spread." },
            { name: "beads", type: "number", default: "5", description: "Hot bead orbs circulating the core (0-20)." },
            { name: "entrance", type: "EmberHuskEntrance", default: '{ scatter: 6, tumble: 6, stagger: 4 }', description: "Mount choreography: shard scatter force, tumble rotation, staggered per-shard timing 0-10." },
            { name: "response", type: "EmberHuskResponse", default: '{ reach: 5, strength: 5, repel: 6, settle: 5 }', description: "Pointer interaction: reach radius, strength, repel force, settle/spring back 0-10." },
            { name: "drift", type: "EmberHuskDrift", default: '{ spin: 3, bob: 4, sway: 4 }', description: "Idle motion: orbital spin, vertical bob, cardinal sway 0-10." },
            { name: "glow", type: "EmberHuskGlow", default: '{ strength: 5, radius: 5, threshold: 5 }', description: "Post glow composite: brightness, spread radius, bloom threshold 0-10." },
            { name: "quality", type: '"low" | "medium" | "high"', default: '"high"', description: "Render scale + physics quality: DPR cap and render-target resolution per tier." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Wrapper style escape hatch." }
        ],
        vibeMeta: {
            behavior: "A raw-WebGL (three.js) scene of a fractured volcanic husk with a single fixed-timestep physics update (PHYS_STEP 1/120 with up to 5 substeps and 0.88 damping) integrated into the render loop. On mount the cracked rock shell (HULL_PLANES polyhedral shards clipped with CLIP_EPS) scatters and tumbles in via per-shard staggered entrance forces (scatter/tumble/stagger); as the shell opens, a molten core shines through with an animated flow noise (ROCK frag + LIGHT_GLSL + NOISE_GLSL) tinted by intensity and hotColor. The scene is dressed with orbiting marker crosses (CROSS_ARM geometry, WAIST/BEVEL), a free-floating debris field, and hot bead orbs. Pointer response maps pointer reach/strength into a repel force on the shell shards that springs back via settle; idle drift adds spin, bob and sway. A glow pass renders the core through down/up-sampled render targets (PREFILTER/DOWN/UP/COMPOSITE) with strength, radius and threshold, and the frame buffer is DPR-capped per quality tier. The whole loop is gated on visibility and the canvas resizes with its host.",
            states: { from: "a dark cracked rock shell frozen in place under a cold sky", to: "shards scatter and tumble in on a staggered entrance to reveal the molten ember core, ringed by crosses, beads and debris under pointer repel" },
            cssProperties: ["three.js", "webgl scene", "fixed-timestep rigid-body physics", "polyhedral hull clipping", "noise-lit rock fragment shaders", "down/up-sampled glow render targets", "pointer reach/repel", "idle spin/bob/sway drift", "quality-tier dpr cap", "visibility gate", "resize observer"],
            description: "A dark WebGL scene of a fractured volcanic husk: a cracked rock shell that tumbles open with a staggered scatter entrance to reveal a molten ember core, ringed by drifting marker crosses, hot beads and debris, responding to pointer reach with a spring-repel on the shell.",
            libraries: ["react", "three"],
            requirements: ["WebGL", "three", "requestAnimationFrame loop", "fixed-timestep physics (120hz, 5 substeps)", "RenderTarget glow compositing", "ResizeObserver + devicePixelRatio cap", "visibilitychange gate"]
        }
    },
    "particle-loader": {
        props: [
            { name: "width", type: "number", default: "0 (auto)", description: "Explicit canvas width in px; 0 falls back to the host element width." },
            { name: "height", type: "number", default: "0 (auto)", description: "Explicit canvas height in px; 0 falls back to the host element height." },
            { name: "dotColor", type: "string", default: '"#94FD00"', description: "Dot fill color (hex, rgb(), rgba(), named; also seeds the accent pass)." },
            { name: "density", type: "number", default: "300", description: "Dot count (20-300, scaled by a 150-count fibonacci field)." },
            { name: "dotSize", type: "number", default: "100", description: "Dot radius (20-300%)." },
            { name: "speed", type: "number", default: "50", description: "Cycle speed (-100 to 100; negative reverses the orbit)." },
            { name: "spinTurns", type: "number", default: "1", description: "Full turns per cycle (-3 to 3)." },
            { name: "ball", type: "Ball", default: '{ spread: 100, turn: 0, tilt: 0 }', description: "Sphere geometry: spread (shell radius 40-180%), turn (rest yaw -180..180 deg), tilt (rest pitch -90..90 deg)." },
            { name: "pointer", type: "Pointer", default: '{ drag: 100, damping: 20 }', description: "Drag interaction: drag (sensitivity 0-300%) and damping (inertia decay 1-100)." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Wrapper style escape hatch." }
        ],
        vibeMeta: {
            behavior: "A Canvas 2D particle loader. Dots are seeded once on a fibonacci sphere (fib) and converted to polar coords; each frame a bump curve eases them from the equator shell up over the poles, then each dot is yaw/pitch-spun, perspective-projected (PERSPECTIVE 3.5 onto pv/(pv-z)) and painter-sorted by depth so nearer dots draw last. A cached autoFit probes 20 frames to scale the field into its host, and dotScaleFor sizes points by canvas size. Pointer drag reorients the shell with exponential-inertia damping (drag/damping), touchAction none enables one-finger fling on mobile. The canvas DPR-caps at 2 and re-measures the host every frame, so it resizes live without a ResizeObserver. Wrapper is 100%x100% with a 24px floor.",
            states: { from: "a low ring of dots orbiting at the equator", to: "the tether blooms up over the poles into a full converging shell, reorientable by dragging with momentum" },
            cssProperties: ["canvas 2d", "fibonacci-sphere seeded dots", "polar swell bump easing", "perspective projection + depth fade", "painter sort by depth", "cached auto-fit sizing", "size-based dot scale", "pointer drag with exponential inertia", "dpr cap", "per-frame host re-measure"],
            description: "A fluorescent particle tether loader: dots bloom up from the equator over a fibonacci-sphere shell with perspective depth and auto-fit sizing, and you can drag to spin the whole field with momentum.",
            libraries: ["react"],
            requirements: ["Canvas 2D", "requestAnimationFrame loop", "Pointer Capture API", "devicePixelRatio cap", "pointer events + touchAction none"]
        }
    },
    "neon-border": {
        props: [
            { name: "color", type: "string", default: '"#CC9149"', description: "Border arc color (hex, rgb(), rgba(), or named; converted to rgba for the gradient stops)." },
            { name: "rounded", type: "number", default: "24", description: "Border corner radius in % of the smaller side (0-100)." },
            { name: "thickness", type: "number", default: "6", description: "Border arc thickness in px (1-10)." },
            { name: "borderSize", type: "number", default: "50", description: "Arc length as % of the perimeter (0-100; short arcs get soft fade tails)." },
            { name: "glow", type: "number", default: "100", description: "Glow strength (0-100): adds layered blurred plus-lighter copies that widen the reach of the arc." },
            { name: "movement", type: '"continuous" | "step"', default: '"continuous"', description: "Motion style: continuous glide around the perimeter, or step that beats corner-to-corner with a springy ease." },
            { name: "speed", type: "number", default: "16", description: "Motion speed (0-20 scale; 0 freezes the arc)." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Wrapper style escape hatch (merged over the 100%x100% flexShrink-0 root)." }
        ],
        vibeMeta: {
            behavior: "Two mirrored conic-gradient arc groups (A at lap 0, B at lap 0.5) are masked to the border region via content-box/border-box xor mask, then animated along the element perimeter. Each frame builds a conic gradient from samples of the arc: withAlpha converts the color to rgba so stops can fade the tail, and the perimeter helper maps a 0-1 lap position to an angle on the box border so the arc wraps corners. The layout runs in a single rAF loop, reading live props via a ref (speed/movement/borderSize/color), and only writes the --arc custom property when speed > 0. Glow is three blurred plus-lighter layers at increasing blur/reach when glow > 0. A ResizeObserver tracks the host box so the arc geometry matches the real border; the root is 100%x100% flexShrink-0 and rounded to the requested radius.",
            states: { from: "a short bright neon arc sitting on one edge of the border", to: "the arc glides around all four corners, leaving a fading tail where it has been" },
            cssProperties: ["conic-gradient built from perimeter samples", "content-box/border-box xor webkit mask", "plus-lighter blend", "blur glow layers", "custom property --arc", "resize observer", "requestAnimationFrame loop", "fade tails via rgba alpha"],
            description: "A moving neon border arc: a short glowing dash built from a per-frame conic gradient skims around the rim of any element with a fading tail and soft layered glow, gliding continuously or stepping corner to corner.",
            libraries: ["react"],
            requirements: ["React", "CSS mask (content-box/border-box xor)", "conic-gradient animation", "ResizeObserver", "requestAnimationFrame loop", "mix-blend-mode plus-lighter"]
        }
    },
    "matrix-rain": {
        props: [
            { name: "background", type: "string", default: '"#0A0A0A"', description: "Trail + canvas base colour (hex, rgb/rgba, or named)." },
            { name: "glyphs", type: "string", default: 'DEFAULT_GLYPHS', description: "Character set to rain (unicode + alnum)." },
            { name: "fontSize", type: "number", default: "18", description: "Glyph size in px (10-64)." },
            { name: "speed", type: "number", default: "55", description: "Fall speed (0-100; 0 freezes the rain)." },
            { name: "density", type: "number", default: "65", description: "Rain density 0-100 (column count + trail length)." },
            { name: "wind", type: "number", default: "0", description: "Horizontal drift (-1..1, pushed into each column's drift)." },
            { name: "color", type: "string", default: '"#3D5CFF"', description: "Primary glyph colour (hex, rgb/rgba, or named)." },
            { name: "highlightColor", type: "string", default: '"#FFFFFF"', description: "Leading glyph colour (hex, rgb/rgba, or named)." },
            { name: "fade", type: "number", default: "0.08", description: "Trail decay alpha per frame (0-0.25)." },
            { name: "interactive", type: "boolean", default: "true", description: "Pointer snap + flare on the nearest column." },
            { name: "seed", type: "number", default: "1", description: "Seeded RNG for column placement, speeds and glyphs." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Wrapper style escape hatch (merged over the 100%x100% root)." }
        ],
        vibeMeta: {
            behavior: "A Canvas 2D digital-rain simulation. Columns of glyphs are seeded once by a seeded RNG into a ResizeObserver-sized field and fall at per-column speeds. Each frame paints a translucent background rect (withColor accumulator on background at alpha = fade) to leave the comet trail, then stamps glyphs top-down with the leading cell full-alpha in highlightColor. density maps to column count (capped at 160) and trail length, speed scales the fall, wind drifts columns sideways. Pointer move/down on the wrapper snaps the nearest column's head to the cursor, injects a flash cell and speeds it up. The canvas DPR-caps at 2 and re-bakes on ResizeObserver; prefers-reduced-motion draws one static frame and the rAF loop is parked while the document is hidden.",
            states: { from: "dark canvas awaiting its first seeded frame", to: "perpetual comet streams of brand-blue glyphs with white heads, drifting under wind and flaring at the pointer" },
            cssProperties: ["canvas 2d", "seeded rng column seeding", "per-column comet trails", "translucent-trail compositing", "pointer snap + flash", "dpr cap", "resize observer", "visibilitychange park", "prefers-reduced-motion static frame"],
            description: "An interactive digital-rain backdrop: seeded streams of brand-blue glyphs fall as glowing comet trails with white leading heads, drift under wind, and flare where the pointer touches the nearest column.",
            libraries: ["react"],
            requirements: ["Canvas 2D", "requestAnimationFrame loop", "ResizeObserver", "pointer events with touchAction none", "prefers-reduced-motion support", "visibilitychange gate"]
        }
    },
    "quantum-lattice": {
        props: [
            { name: "background", type: "string", default: '"#0A0A0A"', description: "Canvas background container fill color." },
            { name: "nodeColor", type: "string", default: '"#3D5CFF"', description: "Resting base color of lattice nodes." },
            { name: "activeColor", type: "string", default: '"#00E5FF"', description: "Highlight color for excited and stretched flux strings." },
            { name: "spacing", type: "number", default: "42", description: "Resting cell distance in px between lattice nodes." },
            { name: "springTension", type: "number", default: "0.08", description: "Elastic spring restitution stiffness factor." },
            { name: "damping", type: "number", default: "0.88", description: "Velocity damping friction coefficient." },
            { name: "reach", type: "number", default: "140", description: "Pointer interaction influence radius in pixels." },
            { name: "impulse", type: "number", default: "0.8", description: "Momentum injection strength from pointer motion." },
            { name: "pulseOnClick", type: "boolean", default: "true", description: "Whether clicking or tapping triggers a radial quantum shockwave." },
            { name: "showNodes", type: "boolean", default: "true", description: "Whether to draw circular particle nodes at lattice intersections." },
            { name: "className", type: "string", default: '""', description: "Extra CSS class names applied to the container." },
            { name: "style", type: "React.CSSProperties", default: "undefined", description: "Style escape hatch for the root container element." }
        ],
        vibeMeta: {
            behavior: "A 2D Canvas mass-spring lattice simulating quantum particle flux. Nodes oscillate with Hooke restitution and exponential velocity damping. Pointer motion transfers kinetic momentum to nearby nodes, stretching connections and shifting color from #3D5CFF to #00E5FF; pointer down fires a radial shockwave.",
            states: { from: "resting geometric lattice with subtle zero-point vacuum vibrations", to: "elastic lattice warping under cursor path and propagating circular shockwaves on click" },
            cssProperties: ["canvas 2d", "mass-spring verlet integration", "elastic line tension rendering", "damped harmonic oscillation", "radial quantum shockwave", "dpr cap", "prefers-reduced-motion fallback"],
